...
def build_parser():
    ...
    parser.add_argument('--node_gate_mode', type=int, default=1, choices=[0, 1, 2], help='0=off, 1=partner_substructure_gate, 2=partner_relation_gate')
    parser.add_argument('--update_mode', type=int, default=1, choices=[0, 1, 2], help='0=standard_concat_update, 1=relation_residual_update, 2=bidirectional_relation_residual_update')
...
def test(s1_data_loader, s2_data_loader, model, device):
    ...
    print('\n')
    print('============================== Best Result ==============================')
    print(f'\t\ts1_acc: {s1_acc:.4f}, s1_roc: {s1_auc_roc:.4f}, s1_aupr: {s1_ap:.4f}, s1_f1: {s1_f1:.4f}, s1_precision: {s1_precision:.4f},s1_recall: {s1_recall:.4f},s1_int_ap: {s1_int_ap:.4f},s1_ap: {s1_ap:.4f}')
    print(f'\t\ts2_acc: {s2_acc:.4f}, s2_roc: {s2_auc_roc:.4f}, s2_aupr: {s2_ap:.4f}, s2_f1: {s2_f1:.4f}, s2_precision: {s2_precision:.4f},s2_recall: {s2_recall:.4f},s2_int_ap: {s2_int_ap:.4f},s2_ap: {s2_ap:.4f}')
...
def main():
    ...
    df_ddi_train = pd.concat([
        pd.read_csv(f'/cmy_project/code/hdn-main-um2_all/PRE-twosides-full-adaptation/drugbank_test/DrugBank/cold_start/fold{args.fold}/train.csv'),
        pd.read_csv(f'/cmy_project/code/hdn-main-um2_all/PRE-twosides-full-adaptation/drugbank_test/DrugBank/cold_start/fold{args.fold}/val.csv')
    ], axis=0)
    df_ddi_s1 = pd.read_csv(f'/cmy_project/code/hdn-main-um2_all/PRE-twosides-full-adaptation/drugbank_test/DrugBank/cold_start/fold{args.fold}/s1.csv')
    df_ddi_s2 = pd.read_csv(f'/cmy_project/code/hdn-main-um2_all/PRE-twosides-full-adaptation/drugbank_test/DrugBank/cold_start/fold{args.fold}/s2.csv')
